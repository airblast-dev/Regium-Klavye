...
class TestKeyboard(Keyboard):
    ...
    def apply_color(self, rgb: list[int, int, int] = None) -> tuple[bytearray]:
        self._color_data()
        ...
        for keyboard in self.keyboards:
            for color in colors:
                keyboard.set_key_color(
                    label, red=color[0], green=color[1], blue=color[2]
                )
                with self.subTest(label=label, color=color):
                    self.assertEqual(keyboard[label].get_color(), list(color))
    ...
    def test_set_key_color_invalid_arg(self):
        ...
        for keyboard, value in zip(self.keyboards, test_values):
            ...
            with self.subTest(
                ...
                self.assertRaises(TypeError, keyboard.set_key_color, label, value)

    def test_set_key_color_data(self):
        for keyboard in self.keyboards:
            profile = PROFILES.get((keyboard._vid, keyboard._pid))
            present_keys: list[
            ...
            for key in present_keys:
                ...
                for color in colors:
                    ...
                    data: list[bytearray] = keyboard.apply_color()
