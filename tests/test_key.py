...
class KeyColorTestCase(unittest.TestCase):
    ...
    def test_key_set_color(self):
        ...
        for value in test_values:
            with self.subTest(
                msg="Set colors using color keywords. (red, green, blue)", value=value
            ):
                self.key.set_color(red=value[0], green=value[1], blue=value[2])
            ...
            with self.subTest(
                ...
                self.assertEqual(self.key.get_color(), value)
            with self.subTest(
                msg="Set colors using 3 color values as seperate arguments.",
                value=value,
            ):
                self.key.set_color(*value)
                self.assertEqual(self.key.get_color(), value)
